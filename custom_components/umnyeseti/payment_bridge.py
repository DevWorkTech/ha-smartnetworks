...
class UmnyeSetiPaymentBridgeView(HomeAssistantView):
    ...
    def _headers(*, script: bool = False) -> dict[str, str]:
        csp = "default-src 'none'; style-src 'unsafe-inline'; img-src 'self'; base-uri 'none'; form-action 'self'"
        if script:
    ...
    async def get(
        ...
        subscriber = str(mapped.get("subscriber") or "").strip()
    ...
    @classmethod
    def _form_page(cls, *, account: str, subscriber: str, amount: str, storage_key: str) -> web.Response:
        account_html = html.escape(account, quote=True)
        subscriber_html = html.escape(subscriber or "Нет данных", quote=True)
...
body{{min-height:100vh;display:flex;align-items:flex-start;justify-content:center;padding:28px 16px 22px}}
.wrap{{width:min(100%,430px)}}
.brand{{display:flex;align-items:center;gap:12px;margin:0 2px 15px}}
.logo{{width:46px;height:46px;border-radius:14px;display:grid;place-items:center;background:linear-gradient(180deg,#26c6ff 0%,#03a9f4 100%);box-shadow:0 8px 20px rgba(3,169,244,.28);position:relative;overflow:hidden;flex:0 0 auto}}
.logo:before{{content:"";position:absolute;inset:0;background:radial-gradient(circle at 50% 34%,rgba(255,255,255,.30),rgba(255,255,255,0) 58%)}}
.logo svg{{position:relative;z-index:1;width:25px;height:25px;display:block;fill:none;stroke:#fff;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}}
...
.subscriber-input{{font-size:14px}}
...
.note-content{{display:inline-flex;align-items:center;justify-content:center;gap:6px;line-height:1.35}}
.note-icon{{display:inline-flex;align-items:center;justify-content:center;line-height:1;font-size:13px;flex:0 0 auto}}
...
@media(max-width:420px){{body{{padding:16px 12px 14px}}.top{{padding:21px 19px 12px}}form{{padding:8px 19px 20px}}.note{{padding:14px 19px 17px}}}}
...
<body>
<div class="wrap">
  <div class="brand"><div class="logo" aria-hidden="true"><svg viewBox="0 0 24 24" role="img"><rect x="3.5" y="6" width="17" height="12" rx="2.5"></rect><path d="M6.5 10.5h11"></path><path d="M8 15h3.5"></path><path d="M16.8 15.2c0 1.2-.9 2.1-2.1 2.1s-2.1-.9-2.1-2.1.9-2.1 2.1-2.1 2.1.9 2.1 2.1Z"></path></svg></div><div><div class="brand-title">Умные Сети</div><div class="brand-sub">Home Assistant</div></div></div>
  <div class="card">
    <div id="form-view">
      <div class="top"><h1>Оплата</h1><p class="intro">Проверьте данные перед совершением платежа</p></div>
      <form id="payment-form" method="post" autocomplete="off">
        ...
        <div class="field"><label for="subscriber">Абонент</label><input id="subscriber" class="subscriber-input" value="{subscriber_html}" disabled></div>
      ...
      <div class="note"><span class="note-content"><span class="note-icon" aria-hidden="true">🔒</span><span>После нажатия откроется защищённая страница оплаты</span></span></div>
    </div>
...
<link rel="icon" href="/static/icons/favicon.ico">
<link rel="apple-touch-icon" href="/static/icons/favicon-192x192.png"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="light dark"><title>Переход к оплате</title>
...
<html lang="ru"><head><meta charset="utf-8">
<link rel="icon" href="/static/icons/favicon.ico">
<link rel="apple-touch-icon" href="/static/icons/favicon-192x192.png"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="light dark"><title>Умные Сети — Ошибка оплаты</title>
